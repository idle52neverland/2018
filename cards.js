...
function showFilters(category) {
  ...
  filterResults(); // 카테고리 진입 시 필터 초기 적용
}
...
function filterResults() {
  const year = document.querySelector(".filter-btn[data-type='year'].active")?.dataset.value;
  const member = document.querySelector(".filter-btn[data-type='member'].active")?.dataset.value;
  const searchText = document.getElementById("subSearchInput").value.toLowerCase();
  ...
  cards.forEach(card => {
    ...
    const matchesMember = !member || cardMember === member;
    const matchesSearch = !searchText || title.includes(searchText);

    card.style.display = matchesYear && matchesMember && matchesSearch ? "block" : "none";
  });
}
